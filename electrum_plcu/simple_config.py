...
class SimpleConfig(Logger):
    ...
    def convert_version_3(self):
        ...
        if isinstance(base_unit, str):
            ...
            map_ = {'plcu':8, 'mplcu':5, 'uplcu':2, 'bits':2, 'sat':0}
    ...
    def get_wallet_path(self, *, use_gui_last_wallet=False):
        ...
        old_path = os.path.join(self.path, "electrum-plcu.dat")
    ...
    def estimate_fee_for_feerate(cls, fee_per_kb: Union[int, float, Decimal],
        ...
        fee_per_kb = FEERATE_DEFAULT_RELAY
        fee_per_byte = fee_per_kb / 1000
...
def read_user_config(path):
    """Parse and store the user config settings in electrum-plcu.conf into user_config[]."""
    if not path:
        return {}
