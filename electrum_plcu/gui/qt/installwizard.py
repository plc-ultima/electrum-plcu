...
from electrum_plcu.wallet import Wallet, Abstract_Wallet
from electrum_plcu.storage import WalletStorage, StorageReadWriteError
from electrum_plcu.util import UserCancelled, InvalidPassword, WalletFileException, get_new_wallet_name
from electrum_plcu.base_wizard import BaseWizard, HWD_SETUP_DECRYPT_WALLET, GoBack, ReRunDialog
...
from electrum_plcu.plugin import run_hook, Plugins
...
class InstallWizard(QDialog, MessageBoxMixin, BaseWizard):
    ...
    def __init__(self, config: 'SimpleConfig', app: QApplication, plugins: 'Plugins', *, gui_object: 'ElectrumGui'):
        ...
        self.setWindowTitle('Electrum-PLCU  -  ' + _('Install Wizard'))
        self.app = app
        self.config = config
        ...
        self.set_icon('electrum-plcu.png')
        self.show()
        self.raise_()
    ...
    def select_storage(self, path, get_wallet_from_daemon) -> Tuple[str, Optional[WalletStorage]]:
        ...
        self.set_layout(vbox, title=_('Electrum-PLCU wallet'))
    ...
    def text_input(self, title, message, is_valid, allow_multi=False, with_tp=False):
        slayout = KeysLayout(parent=self, header_layout=message, is_valid=is_valid,
                             allow_multi=allow_multi, config=self.config, with_tp=with_tp)
    ...
    @wizard_dialog
    def add_xpub_dialog(self, title, message, is_valid, run_next, allow_multi=False, show_wif_help=False, with_tp=False):
        ...
        return self.text_input(title, header_layout, is_valid, allow_multi, with_tp=with_tp)
