...
from electrum_plcu.invoices import (PR_TYPE_ONCHAIN, PR_TYPE_LN, PR_DEFAULT_EXPIRATION_WHEN_CREATING,
                                   PR_PAID, PR_UNKNOWN, PR_EXPIRED, PR_INFLIGHT,
                                   LNInvoice, pr_expiration_values, Invoice, OnchainInvoice)
from electrum_plcu import bitcoin, constants
from electrum_plcu.transaction import tx_from_any, PartialTxOutput
from electrum_plcu.util import (parse_URI, InvalidBitcoinURI, TxMinedInfo, maybe_extract_bolt11_invoice,
...
from electrum_plcu.gui.kivy import KIVY_GUI_PATH
from electrum_plcu.gui.kivy.i18n import _

if TYPE_CHECKING:
    from electrum_plcu.gui.kivy.main_window import ElectrumWindow
...
class SendScreen(CScreen, Logger):
    ...
    def read_invoice(self):
        ...
        if not address:
            self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a PLC Ultima address or a payment request'))
            return
        if not self.amount:
            ...
            if self.is_lightning:
                ...
                if self.payment_request:
                    ...
                    if not bitcoin.is_address(address):
                        self.app.show_error(_('Invalid PLC Ultima Address') + ':\n' + address)
                        return
...
class ReceiveScreen(CScreen):
    ...
    def get_URI(self):
        from electrum_plcu.util import create_bip21_uri
        amount = self.app.get_amount(self.amount)
