...
class BitcoinAverage(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        return dict([(r.replace("PLCU", ""), Decimal(json[r]['last']))
...
class BitcoinVenezuela(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        rates = [(r, json['PLCU'][r]) for r in json['PLCU']
                 if json['PLCU'][r] is not None]  # Giving NULL sometimes
        return dict(rates)
    ...
    async def request_history(self, ccy):
        json = await self.get_json('api.bitcoinvenezuela.com',
                             "/historical/index.php?coin=PLCU")
        return json[ccy +'_PLCU']


class Bitfinex(ExchangeBase):

    async def get_rates(self, ccy):
        json = await self.get_json('api.bitfinex.com', '/v1/pubticker/plcuusd')
...
class Bitso(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.bitso.com', '/v3/ticker/?book=plcu_mxn')
...
class BitStamp(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        if ccy in CURRENCIES[self.name()]:
            json = await self.get_json('www.bitstamp.net', f'/api/v2/ticker/plcu{ccy.lower()}/')
...
class Coinbase(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coinbase.com',
                             '/v2/exchange-rates?currency=PLCU')
...
class CoinCap(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coincap.io', '/v2/rates/plcultima/')
    ...
    async def request_history(self, ccy):
        ...
        history = await self.get_json('api.coincap.io',
                                      '/v2/assets/plcultima/history?interval=d1&limit=2000')
...
class CoinGecko(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coingecko.com', '/api/v3/coins/plcultima?localization=false&tickers=false&community_data=false&developer_data=false&sparkline=false')
    ...
    async def request_history(self, ccy):
        history = await self.get_json('api.coingecko.com',
                                      '/api/v3/coins/plcultima/market_chart?vs_currency=%s&days=max' % ccy)
...
class CoinSpot(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        return {'AUD': Decimal(json['prices']['plcu']['last'])}
...
class GoCoin(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        plcu_prices = json['prices']['PLCU']
        return dict([(r, Decimal(plcu_prices[r])) for r in plcu_prices])
...
class HitBTC(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ccys = ['USD']
        json = await self.get_json('api.hitbtc.com', '/api/2/public/ticker/PLCU%s' % ccy)
...
class Kraken(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        pairs = [k for k in dicts['result'] if k.startswith('XPLCUZ')]
        json = await self.get_json('api.kraken.com',
                             '/0/public/Ticker?pair=%s' % ','.join(pairs))
        ccys = [p[5:] for p in pairs]
        result = dict.fromkeys(ccys)
        result[ccy] = Decimal(json['result']['XPLCUZ'+ccy]['c'][0])
        return result
    ...
    async def request_history(self, ccy):
        query = '/0/public/OHLC?pair=PLCU%s&interval=1440' % ccy
        json = await self.get_json('api.kraken.com', query)
        history = json['result']['XPLCUZ'+ccy]
...
class OKCoin(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('www.okcoin.com', '/api/v1/ticker.do?symbol=plcu_usd')
...
class MercadoBitcoin(ExchangeBase):
    ...
    async def get_rates(self,ccy):
        json = await self.get_json('www.mercadobitcoin.net', '/api/plcu/ticker/')
...
class QuadrigaCX(ExchangeBase):
    ...
    async def get_rates(self,ccy):
        json = await self.get_json('api.quadrigacx.com', '/v2/ticker?book=plcu_cad')
...
class Bitragem(ExchangeBase):
    ...
    async def get_rates(self,ccy):
        json = await self.get_json('api.bitragem.com', '/v1/index?asset=PLCU&market=BRL')
