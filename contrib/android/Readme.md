...
```
$ adb -d install -r dist/Electrum-PLCU-*-arm64-v8a-debug.apk
$ adb shell monkey -p org.electrum_plcu.electrum_plcu 1
```
...
```
adb logcat | grep -F "`adb shell ps | grep org.electrum_plcu.electrum_plcu | cut -c14-19`"
```
...
Run electrum with the `-g` switch: `electrum-plcu -g kivy`
...
```
$ adb shell
$ run-as org.electrum_plcu.electrum_plcu ls /data/data/org.electrum_plcu.electrum_plcu/files/data
$ run-as org.electrum_plcu.electrum_plcu cp /data/data/org.electrum_plcu.electrum_plcu/files/data/wallets/my_wallet /sdcard/some_path/my_wallet
```
