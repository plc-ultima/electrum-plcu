...
APPDIR="$BUILDDIR/electrum-plcu.AppDir"
...
APPIMAGE="$DISTDIR/electrum-plcu-$VERSION-x86_64.AppImage"
...
cp -f "$PROJECT_ROOT/electrum_plcu/libsecp256k1.so.0" "$APPDIR/usr/lib/libsecp256k1.so.0" || fail "Could not copy libsecp to its destination"
...
info "preparing electrum-plcu-locale."
(
    cd "$PROJECT_ROOT"
    git submodule update --init

    pushd "$CONTRIB"/deterministic-build/electrum-plcu-locale
    if ! which msgfmt > /dev/null 2>&1; then
        fail "Please install gettext"
    fi
    # we want the binary to have only compiled (.mo) locale files; not source (.po) files
    rm -rf "$PROJECT_ROOT/electrum_plcu/locale/"
    for i in ./locale/*; do
        dir="$PROJECT_ROOT/electrum_plcu/$i/LC_MESSAGES"
        mkdir -p $dir
...
cp "$PROJECT_ROOT/electrum-plcu.desktop" "$APPDIR/electrum-plcu.desktop"
cp "$PROJECT_ROOT/electrum_plcu/gui/icons/electrum-plcu.png" "$APPDIR/electrum-plcu.png"
