...
if [ ! -z "$ELECBUILD_COMMIT" ] ; then
    ...
    FRESH_CLONE="$CONTRIB_WINE/fresh_clone/electrum-plcu" && \
...
sudo docker run -it \
    ...
    -v "$PROJECT_ROOT_OR_FRESHCLONE_ROOT":/opt/wine64/drive_c/electrum-plcu \
    --rm \
    --workdir /opt/wine64/drive_c/electrum-plcu/contrib/build-wine \
