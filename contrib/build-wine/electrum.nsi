...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-PLCU"
  !define PRODUCT_WEB_SITE "https://github.com/plc-ultima/electrum-plcu"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-plcu-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "c:\electrum-plcu\electrum_plcu\gui\icons\electrum.ico"
...
Section
  ...
  File /r "dist\electrum-plcu\*.*"
  File "c:\electrum-plcu\electrum_plcu\gui\icons\electrum.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-plcu-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-plcu-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-plcu-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-plcu-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-plcu-${PRODUCT_VERSION}.exe" 0


  ;Links bitcoin: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\plcultima" "" "URL:plcultima Protocol"
  WriteRegStr HKCU "Software\Classes\plcultima" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\plcultima" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\plcultima\shell\open\command" "" "$\"$INSTDIR\electrum-plcu-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\plcultima"
