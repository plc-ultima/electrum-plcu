...
if [ -z "$1" ]; then
    echo "Usage: $0 Electrum-PLCU.app"
    exit -127
fi
...
if ! which ${genisoimage} > /dev/null 2>&1; then
	mkdir -p /tmp/electrum-plcu-macos
	cd /tmp/electrum-plcu-macos
...
rm -rf /tmp/electrum-plcu-macos/image > /dev/null 2>&1
mkdir /tmp/electrum-plcu-macos/image/
cp -r $1 /tmp/electrum-plcu-macos/image/
...
${genisoimage} \
    ...
    -dir-mode 0755 \
    -apple \
    -o Electrum-PLCU_uncompressed.dmg \
    /tmp/electrum-plcu-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg Electrum-PLCU_uncompressed.dmg electrum-plcu-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum-PLCU_uncompressed.dmg

echo "Done."
sha256sum electrum-plcu-$VERSION.dmg
